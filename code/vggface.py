...
class Model():
    ...
    def get_conv_filter(self, name):
        w = np.load("../vggface/" + str(name) + "_filter.npy")
        return tf.Variable(w, dtype=tf.float32, name="filter")

    def get_bias(self, name):
        b = np.load("../vggface/" + str(name) + "_bias.npy")
        return tf.Variable(b, dtype=tf.float32, name="bias")

    def get_fc_weight(self, name):
        cw = np.load("../vggface/" + str(name) + "_weight.npy")
        return tf.Variable(cw, dtype=tf.float32, name="weight")
    ...
    def build(self, rgb, train=False):
        ...
        with tf.variable_scope('vggface_' + self.name):
